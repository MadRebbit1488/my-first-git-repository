...
-Element.classList - , додавати, видаляти класи елемента.
          ...
          *entries() - виводить весь список класів
          *forEach() - ітерація
...
В консоль виведеться :
    1. CSS-позиціонування (position is absolute, relative, fixed або sticky), 
    2. Або <td>, <th>, або <table>
    3. Або якщо немає нічього із вищє перечисленого <body>
Тобто якщо є:
      <div class="parents"> 
          <div>  </div> 
      </div> 
і батьківсткий <div class="parents">  має властивість (position is absolute, relative, fixed або sticky) , то виведеться батьківський <div>, в іншому випадку <body>
...
Більшість відповідних методів JS працюють з однією із двох систем координат: 
    ...
    *Відносно документа - схоже на position:absolute відносно <body>, обчислюється від верхнього лівого кута документа
    Позначається pageX / pageY

Коли сторінка прокручена до самого початку, то верхній лівий кут вікна точно збігається з верхнім лівим кутом доумента, тому їх системи координат також збігаються. Але якщо документ прокрутити, то координати елементів відносно вікна змінюються, так як вони рухаються, а координати відносно документа залишаються сталими
